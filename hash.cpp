...
int hashTable::insert(const string &key, void *pv){ // Check pointers
	...
	while(filled>=capacity/2){  // Changed 2*capacity to capacity/2
	...
	while(data[index].isOccupied == true){
		...
		index++;
		if(index>=capacity-1){ // Loops the index back around
			index = 0;  // Changed index>capacity --> index>=capacity-1;
		}
	}
